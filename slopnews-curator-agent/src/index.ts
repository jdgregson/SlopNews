...
export default {
  async fetch(
    ...
    try {
      ...
      // Check API key (cron triggers have special header)
      const isCron = request.headers.get("cf-cron");
      const providedKey = url.searchParams.get("key");

      if (!isCron && (!providedKey || providedKey !== env.AGENT_ACCESS_KEY)) {
        return new Response(
      ...
      const ai = env.AI;
      const headlineResponse = await ai.run(HEADLINE_MODEL as any, {
        messages: [
